...
2. **PATH shortcuts.** Create one-word commands: verify `~/.local/bin` is on `PATH` (fall back to `/usr/local/bin` if writable, or ask), then `ln -sf "${CLAUDE_PLUGIN_ROOT}/skills/fableseek/fableseek.sh" ~/.local/bin/fableseek`, `ln -sf "${CLAUDE_PLUGIN_ROOT}/skills/fableseek/fableseek-cost" ~/.local/bin/fableseek-cost`, and `ln -sf "${CLAUDE_PLUGIN_ROOT}/skills/ask/fableseek-ask" ~/.local/bin/fableseek-ask`. Confirm all three resolve with `command -v`.
